...
export default function HW1Tasks() {
  ...
  const invalid =
    trimmed.length < 1 || trimmed.length > 120
  const invalidReason =
    trimmed.length < 1
      ? 'Title is required.'
      : trimmed.length > 120
  ...
  async function addTask(e) {
    ...
    try {
      const optimistic = { _id: 'tmp-' + Math.random(), title: trimmed, completed: false, optimistic: true }
      setItems(prev => [optimistic, ...prev]); setText('')
      const task = await authed('/api/tasks', { method: 'POST', body: { title: trimmed } })
      setItems(prev => prev.map(t => (t._id === optimistic._id ? task : t)))
      toast({ title: 'Added', variant: 'success' })
      inputRef.current?.focus()
    } catch (e) {
      setError(e.message); shake('task-form')
    }
  }
  ...
  async function remove(id) {
    ...
    try { await authed(`/api/tasks/${id}`, { method: 'DELETE' }) } catch {}
  }
  ...
  return (
    <div className="stack">
      ...
      <form id="task-form" className="card stack" onSubmit={addTask} style={{ gap: 8 }}>
        <div className="row">
          <input
            ref={inputRef}
        ...
        {invalid && <span className="label" style={{ color: 'crimson' }}>{invalidReason}</span>}
      </form>
